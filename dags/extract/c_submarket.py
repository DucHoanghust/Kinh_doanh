...
def extract_c_submarket():
    source_operator = OracleHookThick(conn_id="SOURCE_ORACLE")
    staging_operator = PostgresOperators(conn_id="STAGING_POSTGRES")
    

    df = source_operator.get_data_to_pandas("select c_submarket_id, c_market_id, name, value, isactive,created, updated from c_submarket")
    print(df.columns)
    print(df.shape)
    df.columns = [col.lower() for col in df.columns]
    staging_operator.save_data_to_postgres(
        df,
        table_name="c_submarket",
        schema="kd_stag",
        if_exists="replace"
