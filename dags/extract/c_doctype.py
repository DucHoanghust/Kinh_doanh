...
def extract_c_doctype():
    source_operator = OracleHookThick(conn_id="SOURCE_ORACLE")
    staging_operator = PostgresOperators(conn_id="STAGING_POSTGRES")
    

    df = source_operator.get_data_to_pandas("select c_doctype_id,name, printname, docbasetype,issotrx, isactive,created, updated from c_doctype")
    print(df.columns)
    print(df.shape)
